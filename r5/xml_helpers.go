...
// MarshalXML writes an unknown resource back exactly as it arrived.
//
// Verbatim, like the JSON side: the library cannot interpret the resource, so
// reformatting it would be inventing a representation for something it does not
// model. The element is re-emitted through ",innerxml", which passes its content
// through untouched rather than re-parsing and re-serializing it.
...
// define, keeping the element verbatim so it survives the round trip.
//
// The bytes are the element as it arrived, rebuilt token by token — the same
// approach xmlDecodeRawXHTML uses for narrative. What that cannot carry is a
