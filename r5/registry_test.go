...
func TestNewResource(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resource, err := r5.NewResource(tt.resourceType)
			if tt.wantErr {
				assert.Error(t, err)
...
func TestUnmarshalResource(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resource, err := r5.UnmarshalResource([]byte(tt.json))
			if tt.wantErr {
				assert.Error(t, err)
				if tt.errContains != "" {
...
func TestUnmarshalResourceRoundTrip(t *testing.T) {
	...
	patient, ok := resource.(*r5.Patient)
	require.True(t, ok, "expected *r5.Patient, got %T", resource)
	...
	assert.Equal(t, r5.PtrSlice("John"), patient.Name[0].Given)
}
...
func TestGetResourceType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := r5.GetResourceType([]byte(tt.json))
			if tt.wantErr {
				assert.Error(t, err)
				if tt.errContains != "" {
					assert.Contains(t, err.Error(), tt.errContains)
...
func TestIsKnownResourceType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := r5.IsKnownResourceType(tt.resourceType)
			assert.Equal(t, tt.want, got)
		})
	}
}
...
func TestAllResourceTypes(t *testing.T) {
	...
	// Should have many resource types (R5 has ~160)
