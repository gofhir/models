...
| Tarea | Por qué |
|---|---|
| **`UnknownResource` de reserva** *(nuevo)* | **✅ Hecha (JSON).** El fallo era peor de lo descrito: `Bundle.entry` quedaba con las entradas leídas antes del error **y** `Unmarshal` devolvía error, así que quien solo lo logueara se quedaba con un Bundle silenciosamente incompleto —2 de 3 entradas—, más difícil de notar que un fallo limpio. Ahora el JSON crudo se conserva byte a byte y el round-trip preserva los miembros que la versión no modela. `NewResource` sigue fallando: pedir un tipo por nombre y recibir uno vacío no responde a nada. **XML también lo conserva ya**, cerrando la asimetría: el elemento se captura con la misma técnica que la narrativa y se re-emite por `,innerxml`. **No es byte a byte**, a diferencia del JSON: la captura reconstruye desde el flujo de tokens, que no distingue cómo se escribió un elemento vacío —`<self/>` vuelve como `<self></self>`, el mismo elemento—. Ningún miembro ni contenido se pierde, y un segundo round-trip ya no cambia nada. El riesgo anotado —los namespaces— **se materializó**: la primera versión declaraba `xmlns` dos veces por pasar los atributos a la vez en el `StartElement` y en el campo `,any,attr`; lo detectó el test escrito precisamente para eso. Un recurso solo lleva `Raw` **o** `RawXML`: convertir entre formatos exigiría conocer su estructura, que es lo que lo hace desconocido, así que pedir el otro formato es un error explicado. |
| ~~**Validación en el límite**~~ | ❌ **Descartada por la misma razón.** Decidir qué documento es válido no le toca a la capa de tipos. También se retiró el `IsValid()` de los enums, que había entrado en la 2.5.0: aunque solo se genera para bindings `required` —para `extensible` o `preferred` el campo queda como `*string`, porque ahí un código de fuera **sí** es válido—, un `false` se lee con demasiada facilidad como «este recurso es inválido», que no es lo que dice y que esta librería no está en posición de afirmar. La tabla sigue exportada: quien quiera esa comprobación puede escribirla y hacerse cargo de lo que significa. |
| **Enums con identidad completa** | **✅ Hecha.** `Display()`, `System()`, `Coding()`, `IsValid()` y `<Tipo>Values()` en **los 206 enums de r4** (215 r4b, 237 r5), +45.712 líneas. El display estaba en la especificación y se emitía **solo como comentario**, ilegible en runtime; el system se descartaba entero. Se añadió `System` a `parser.ParsedCode` —tomado de `include.System` o de la URL del CodeSystem— y **ninguna de las 1.107 entradas de r4 quedó sin él**. `Definition` se dejó fuera: es texto largo y multiplicaría el tamaño sin un caso de uso claro. La tabla es por código, no por tipo, porque un ValueSet puede incluir códigos de varios CodeSystems. |
| **Accessors de choice type** | **Parcial.** La exclusividad está hecha en los builders: un `Set<Variante>` limpia las demás, incluidos los `_field` de las primitivas. Cubre **218 grupos** de los 645 (r4 61/186), porque los builders solo existen en la raíz de los recursos — los choice de backbones y datatypes no tienen builder, `Extension.value[x]` con 71 variantes entre ellos. Los accessors de **lectura** siguen pendientes. |
| **Extensiones por URL** | **✅ Hecha.** `GetExtensionByURL`, `GetExtensionsByURL`, `HasExtensionByURL` y `GetModifierExtensionByURL` en **los 660 tipos de r4 con campo `extension`** (678 en r4b, 793 en r5): recursos, datatypes, backbones y la propia `Extension`, que es como se recorre una extensión compleja. Más las funciones libres `ExtensionByURL`/`ExtensionsByURL`/`HasExtensionByURL` para un `[]Extension` suelto. Ambas búsquedas devuelven **punteros**: con valores, el plural sería silenciosamente de solo lectura mientras el singular no lo es. +56.750 líneas. El modificador va aparte a propósito: cambia el significado del elemento, y quien no lo reconozca no debe procesarlo. |
...
| **Errores con path FHIR** | **Parcial, y la mitad ya estaba hecha.** El `.Alias` filtraba solo en los tipos con campo polimórfico —`Parameters.parameter.Alias.name`—, no en todos; corregido sustituyéndolo por el **path FHIR**, `Parameters.parameter.name`, conservando la cadena para `errors.Is`/`As`. El resto de errores **ya nombraban tipo y campo** (`Patient.active`) y los del dispatcher **ya llevaban índice** (`contained[1]`). Lo que sigue faltando es el índice dentro de un array de datatypes —`Patient.name[1].family`— que `encoding/json` no reporta y exige decodificación propia; sin él no se puede poblar `OperationOutcome.issue.expression`. |
...
| ~~**`Validate()` generado**~~ | ❌ **Descartado.** No es de esta librería. Ver abajo. |



### La validación no vive aquí — **decisión**

El plan proponía generar `Validate()` en `models` «coordinando el reparto con `gofhir/validator`». Se descarta, y el propio plan ya contenía las tres piezas para verlo:

- La línea 293 dice que **«los structs no validan y eso vive en `gofhir/validator`»**.
- La tabla de riesgos lista **«la fase 8 duplica el alcance de `gofhir/validator`»**.
- Y aun así la fase 8 lo proponía. No era un plan coherente en este punto, sino una idea sin resolver.

Los datos lo cierran. `gofhir/validator` va por **v1.21.0** (agosto de 2026) frente a la 2.6.0 de `models`: es el proyecto maduro, y sería el aprendiz quien duplicaría su trabajo.

**No hay ciclo de importación** —`validator` no depende de `models`, sino de `fhirpath`, `ucum` y `x/net`, y trabaja sobre el JSON crudo—, así que el argumento no es técnico. Es peor que un ciclo: dos implementaciones de validación FHIR en la misma organización pueden dar **veredictos distintos para el mismo documento** según cuál use quien llama. Eso es peor que no validar aquí.

Queda igualmente descartada la «validación en el límite», cablear la comprobación al unmarshal, por el mismo motivo: decidir qué es válido no le toca a la capa de tipos.

**También se retiró `IsValid()`.** Había entrado en la 2.5.0 con el argumento de que solo describe el tipo, y es verdad que se genera únicamente para bindings `required` —en `extensible` o `preferred` el campo es `*string`, porque allí un código externo es válido—. Pero la frontera es demasiado fina: un `false` se lee como «este recurso es inválido», que no es lo que afirma. La tabla `<Tipo>Table` queda exportada, así que la comprobación se puede escribir fuera, donde quien la haga se hace cargo de lo que significa.
