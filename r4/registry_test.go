...
func TestNewResource(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resource, err := r4.NewResource(tt.resourceType)
			if tt.wantErr {
				assert.Error(t, err)
...
func TestUnmarshalResource(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resource, err := r4.UnmarshalResource([]byte(tt.json))
			if tt.wantErr {
...
func TestUnmarshalResourceRoundTrip(t *testing.T) {
	...
	require.True(t, ok, "expected *r4.Patient, got %T", resource)
	...
	assert.Equal(t, r4.PtrSlice("John"), patient.Name[0].Given)
}
...
func TestGetResourceType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := r4.GetResourceType([]byte(tt.json))
			if tt.wantErr {
				assert.Error(t, err)
				if tt.errContains != "" {
					assert.Contains(t, err.Error(), tt.errContains)
...
func TestIsKnownResourceType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := r4.IsKnownResourceType(tt.resourceType)
			assert.Equal(t, tt.want, got)
		})
	}
}
...
func TestAllResourceTypes(t *testing.T) {
	...
	// Should have many resource types (R4 has ~150)
